...
class TaskList extends Component {
    ...
    tasks &&
      Promise.all(
        ...
        Object.keys(division).map(div =>
          sortingByType(division[div], sorting, sortingType).map(
            (task, index) => {
  ...
  getDateDivision = date => {
    ...
    else if (+newDate <= +moment(now).add(1, 'days').endOf('day'))
      text = translations.tomorrow;
    else if (newDate <= moment(now).endOf(week)) text = translations.thisWeek;
    else if (+newDate <= +moment(now).add(1, 'week').endOf(week))
      text = translations.nextWeek;
    else if (newDate <= moment(now).endOf('month'))
      text = translations.thisMonth;
    else if (newDate <= moment(now).add(1, 'month').endOf('month'))
      text = translations.nextMonth;
  ...
  dropdownRenderRow = (rowData, index) => {
    ...
    if (rowData.id === -3) {
      ...
      const amountOfTasks = tasks.filter(
        task => task.category.id === rowData.id,
      );
      data = {
  ...
  getFilterData = () => {
    ...
    return data.filter(({task}, index) => {
      ...
      if (
        searchText.length > 0 &&
        task.name.toLowerCase().indexOf(searchText) < 0
      ) {
  ...
  renderTaskRow = ({task, div, showDiv}) => {
    ...
    const moveValue = animations[`move${task.id}`]
      ? animations[`move${task.id}`]
      : 0;
            ...
            <ListItem
              ...
              rightElement={
                <View style={styles.rightElements}>
                  <IconToggle
                    color={
                      task.finish ? theme.undoIconColor : theme.doneIconColor
                    }
                    style={{
                      container: {
            ...
            <>
              {dropdownData ? (
                <ModalDropdown
                  ...
                  renderRow={(renderRow, index) =>
                    this.dropdownRenderRow(renderRow, index)
                  }>
        ...
        <View>
          {selectedCategory.name !== translations.finished ? (
            <ActionButton
              hidden={bottomHidden}
              onPress={() =>
                navigation.navigate('ConfigTask', {category: selectedCategory})
              }
              icon="add"
              style={{
...
const mapDispatchToProps = dispatch => ({
  ...
  onFinishTask: (task, endTask, primaryColor, translations, callback) =>
    dispatch(
      actions.finishTask(task, endTask, primaryColor, translations, callback),
    ),
  ...
  onChangeSorting: (sorting, type) =>
    dispatch(actions.changeSorting(sorting, type)),
});
